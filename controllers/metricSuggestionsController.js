...
/**
 * Process unmatched metrics from upload
 * Auto-maps high-confidence matches (≥95%)
 * Creates suggestions for low-confidence matches (≤94%)
 */
async function processUnmatchedMetrics(req, res) {
  try {
      ...
      if (synonymMatch && synonymMatch.confidence >= 0.95) {
        ...
        try {
          ...
          `, [
            userId,
            uploadId,
            synonymMatch.system_id,
            synonymMatch.canonical_name,
            parseFloat(value),
            unit || synonymMatch.canonical_unit,
            testDate || new Date()
          ]);
          ...
          // Log the auto-mapping for audit
          ...
          // If auto-map fails, add to review queue
          needsReview.push({
            original_name: name,
            value: value,
            unit: unit,
            error: 'Auto-map insertion failed'
          });
        }

      } else if (synonymMatch && synonymMatch.confidence >= 0.70) {
        // MANUAL REVIEW: Medium confidence (70-94%)
        ...
        // LOW CONFIDENCE: Get AI suggestions
    ...
    // Save suggestions that need review to database
    ...
    res.status(500).json({
      success: false,
      error: 'Failed to process unmatched metrics',
...
async function getPendingSuggestions(req, res) {
  try {
    ...
    res.status(500).json({
      success: false,
      error: 'Failed to retrieve pending suggestions',
      message: error.message
    });
  }
}
...
/**
 * Approve selected metric mappings
 */
  ...
  try {
    ...
    if (!Array.isArray(approvedMappings) || approvedMappings.length === 0) {
      return res.status(400).json({
        success: false,
        error: 'No mappings provided for approval'
      });
    }
    ...
    // Insert approved mappings into metrics table
    for (const mapping of approvedMappings) {
      ...
      try {
        ...
        `, [
          ...
          canonical_name,
          parseFloat(value),
          unit || catalogMetric.canonical_unit,
          suggestion.test_date || new Date()
        ]);

        insertedMetrics.push(result.rows[0]);

        // Add synonym for future auto-mapping
        await synonymService.addUserSynonym(
          userId,
          original_name,
          canonical_name,
          metric_id
...
/**
 * Reject/dismiss pending suggestions
 */
...
/**
 * Helper: Get AI suggestions for unmatched metric
 */
...
/**
 * Helper: Log auto-mapping for audit trail
 */
